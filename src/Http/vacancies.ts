...
export const VacancyService = {
    async login() {
        const response = await $api.get('/oauth2/password?login=sergei.stralenia@gmail.com&password=paralect123&client_id=2356&client_secret=v3.r.137440105.ffdbab114f92b821eac4e21f485343924a773131.06c3bdbb8446aeb91c35b80c42ff69eb9c457948&hr=0');
        return response
    },
    ...
    async getVacancies(query: QueryType = defaultQuery) {
        ...
        const response = await $api(`/vacancies/?${queryString.join('&')}&count=4&published=1`, {
            headers: {
                'x-secret-key': 'GEU4nvd3rej*jeh.eqp',
                'access_token': 'v3.r.137440105.041d2e6c58c1314e6a2157b49200cf2f1665835e.4838e8f3fc75f96636e9c206e8fbfc1cd8c15dc8',
                'x-api-app-id': 'v3.r.137528818.42ccfb7041ed0160dabfa6a739a631448f0ff724.49cb285f6936b5889d7540c35b5edf9f2d427a0a',
                'client_id': '2388',
            }
        });
        return response
    ...
    async getFavoriteVacancies(ids: Array<number>) {
        const queryParams = ids.map((id, index) => `ids[${index}]=${id}${index !== ids.length - 1 ? '&' : ''}`)
