...
export default function Favorites() {
    ...
    useEffect(() => {
        ...
        if (favoritesVacancies && JSON.parse(favoritesVacancies).length !== 0 && isVacancyChanged) {
            const asyncFunc = async () => {
                setIsLoading(true)
                try {
                    const vacanciesOnPage = JSON.parse(favoritesVacancies).slice((currentPage - 1) * 4, currentPage * 4);
                    const vacanciesArray = await Promise.all(vacanciesOnPage.map(async (vacancyId: number) => {
    ...
    if (vacancies.length === 0) {
        return (
            <div className={styles.not_found_container}>
                ...
                <Link href={'vacancies'} className={styles.link_button}>Поиск вакансий</Link>
            </div>
        )
    }
    ...
    return (
        <div className={styles.container}>
            {vacancies.map(vacancy => {
                return <Vacancy vacancy={vacancy} key={vacancy.id} callback={onVacancyDeleteFromFavorites}/>
            })}
            {totalCount > 4 &&
                <div className={styles.pagination}><PaginationContainer page={currentPage} callback={onChangePage}
                                                                        count={totalCount}/></div>}
        </div>
    )
}
