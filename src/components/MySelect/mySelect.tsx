...
export const MySelect = ({name, value, callback}: PropsType) => {
    ...
    const [options, setOptions] = useState<Array<{ label: string, key: number }>>(defaultMappedCatalogues);

    useEffect(() => {
        const asyncFunc = async () => {
            try {
                const cataloguesResponse: any = await VacancyService.getCatalogues()
                const mappedCatalogues = cataloguesResponse.data.map((catalog: {
                    title_rus: string;
                    url_rus: string;
                    title: string;
                    id_parent: number;
    ...
    const items = options.map((item) => (
        <Menu.Item
            ...
            key={item.label}
            className={`${styles.option} ${item.label === value.label && styles.active_option}`}
        >
            {item.label}
        </Menu.Item>
    ));
        ...
        >
            <Menu.Target>
                <UnstyledButton className={`${styles.control} ${opened && styles.opened}`}>
                    <Group spacing="xs">
                        <span
                            className={`${styles.label} ${value.isHidden && styles.hidden}`}>{value.label}</span>
                    </Group>
                    {!opened
                        ? <div className={styles.chevron_icon}><ArrowDownIcon/></div>
                        : <div className={`${styles.chevron_icon} ${styles.reverse}`}><ArrowDownIcon/></div>}
                </UnstyledButton>
            </Menu.Target>
